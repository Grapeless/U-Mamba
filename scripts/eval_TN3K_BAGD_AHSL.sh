#!/bin/bash
# Usage:
#   bash scripts/eval_TN3K_BAGD_AHSL.sh                        # default checkpoint_best.pth
#   bash scripts/eval_TN3K_BAGD_AHSL.sh checkpoint_final.pth   # specify checkpoint
...
RESULT_DIR="data/nnUNet_results/Dataset705_TN3K/nnUNetTrainerUMambaEnc_BAGD_AHSL__nnUNetPlans__2d"
...
CUDA_VISIBLE_DEVICES=0 nnUNetv2_predict \
    ...
    -d 705 -c 2d -f all \
    -tr nnUNetTrainerUMambaEnc_BAGD_AHSL \
    --disable_tta \
    -chk "${CHECKPOINT}" &&
